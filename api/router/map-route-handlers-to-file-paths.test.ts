...
describe("mapRouteHandlersToFilePaths()", () => {
  test("should return an Map of route handlers to file paths", async () => {
    ...
    let map = await mapRouteHandlersToFilePaths(basePath, paths, { importRouteModule, validate } )
  ...
  test("should throw if the route handler is invalid", async () => {
    ...
    let validate = (_: string, __: unknown) => { throw new Error("Invalid") }

    try {
      await mapRouteHandlersToFilePaths(basePath, paths, { importRouteModule, validate } )
    } catch (e) {
